...
async function adversarialPath(
	...
	console.log(
		`Now that the transacstion was succesfuly submitted, wait ${delayPeriod} blocks after announcement (until block${inclusionPoint5?.height + delayPeriod
		}) ` +
...
async function happyPath(
	...
	console.log(
		`\nNow that the transaction was succesfuly submitted, we will wait ${delayPeriod} blocks after the announcement (until block ${inclusionPoint3?.height + delayPeriod
		}) ` +
		'for the delay periood to pass and execute with proxyAnnounced.' +
		'\nThere is a process in the background that will fire proxyAnnounced to execute the actual balance ' +
